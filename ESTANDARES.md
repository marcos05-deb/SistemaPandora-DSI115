...
*   **Bypass Seguro de AreaScope (`withoutGlobalScopes`):** Todo uso de `withoutGlobalScopes()` o `withoutGlobalScope(AreaScope::class)` debe: (a) limitarse exclusivamente a `exists()`, `count()`, o `selects` explícitos de columnas NO sensibles — **nunca** hidratar el modelo completo ni exponer su resultado en un Resource o prop de Inertia; y (b) llevar un comentario explicativo en el código que justifique por qué es seguro, citando explícitamente esta regla.
